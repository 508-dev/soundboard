...
- Treat `keystore.properties` (gitignored, see `keystore.properties.example`)
  as a secret. Never print its contents or commit a real one. The same goes for
  any `SOUNDBOARD_KEYSTORE_*` or `FDROID_*` environment variable — those are how
  CI passes the same key in.
...
- `scripts/`: thin wrappers around `./gradlew`, plus the two release scripts
  (`sync-version.sh`, `fdroid-publish.sh`).
- `fastlane/metadata/android/`: store copy and release notes. Read by both
  F-Droid paths and pasted into Play. The single source for listing text.
- `fdroid/`: inputs for the self-hosted F-Droid repo, and the f-droid.org
  submission metadata. Two different files for two different systems — the
  header comment in each says which.
- `version.txt`, `CHANGELOG.md`, `.release-please-*.json`: release automation.
  Generated; see below.
...
Releases are automated end-to-end (`docs/deployment.md`). Three ways to break
them that all compile and pass tests:

- **Never hand-edit `versionCode`, `versionName`, `CHANGELOG.md`, or
  `.release-please-manifest.json`.** `version.txt` is the source of truth;
  everything else is generated by `./scripts/sync-version.sh`. CI fails on
  drift.
- **Keep the version literals literal.** `versionCode = 1000` and
  `versionName = "0.1.0"` must stay plain values on their own lines in
  `app/build.gradle.kts`. F-Droid's update bot regex-parses them out of that
  file; an expression silently ends automatic F-Droid releases.
- **Store copy belongs in `fastlane/metadata/android/`**, not in the F-Droid
  metadata YAML and not in the Play Console. Everything else reads from there.

Commit subjects are Conventional Commits and drive the version — see
`CONTRIBUTING.md`. Don't add a new publishing target, or promote Play past the
internal track, without the maintainer asking.
